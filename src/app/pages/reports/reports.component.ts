...
const MOCK_REPORTS: Report[] = [
  {
    id: 1,
    appointmentId: 3,
    patientFullName: 'Anna Francis',
    doctorFullName: 'Mihai Lavretti',
    visitType: 'Seduta Osteopatica',
    scheduledAt: '2025-04-07T10:00:00',
  ...
  },
  {
    id: 2,
    appointmentId: 5,
    patientFullName: 'Alessandru Davini',
    doctorFullName: 'Luca Siretta',
    visitType: 'Valutazione Fitness',
    scheduledAt: '2025-04-12T09:00:00',
  ...
  },
  {
    id: 3,
    appointmentId: 8,
    patientFullName: 'Elena Debuo',
    doctorFullName: 'Simona Ruberti',
    visitType: 'Visita Nutrizionistica',
    scheduledAt: '2025-04-15T11:00:00',
  ...
  },
  {
    id: 4,
    appointmentId: 12,
    patientFullName: 'Marco Lavecri',
    doctorFullName: 'Sandro Scrigoni',
    visitType: 'Visita Medico-Sportiva',
    scheduledAt: '2025-04-17T09:30:00',
  ...
  },
  {
    id: 5,
    appointmentId: 15,
    patientFullName: 'Erica Guella',
    doctorFullName: 'Cristiana Maratti',
    visitType: 'Consulenza Nutrizionale Sportiva',
    scheduledAt: '2025-04-20T14:00:00',
  ...
  },
  {
    id: 6,
    appointmentId: 18,
    patientFullName: 'Nadia Pietri',
    doctorFullName: 'Mihai Lavretti',
    visitType: 'Riatletizzazione e Recupero Funzionale',
    scheduledAt: '2025-04-22T10:30:00',
...
export class ReportsComponent implements OnInit {
  ...
  constructor(
    ...
    private clinicalAppointmentService: ClinicalAppointmentService,
  ...
  loadCompletedAppointments(): void {
    this.clinicalAppointmentService.getAll({ status: 'COMPLETED' }).pipe(
      ...
      // Mostra solo quelli che non hanno ancora un referto
  ...
  save(): void {
    ...
    request$.subscribe({
      next: () => {
        this.alertService.show(this.editingId ? 'Referto aggiornato con successo' : 'Referto creato con successo');
        this.closeFormModal();
        this.load();
      },
      error: () => {
        this.alertService.show('Errore durante il salvataggio del referto', 'error');
      }
    });
  }
  ...
  getVisitTypeBadgeClass(visitType: string): string {
    ...
    if (t.includes('osteopat') || t.includes('riatlet') || t.includes('recupero')) return 'badge-orange';
    if (t.includes('fitness') || t.includes('personal') || t.includes('allenamento') || t.includes('atletica') || t.includes('agonistic')) return 'badge-blue';
    if (t.includes('nutriz') && t.includes('sport')) return 'badge-green';
    if (t.includes('nutriz') || t.includes('piano nutriz') || t.includes('composizione')) return 'badge-pink';
    if (t.includes('medico') || t.includes('sport') || t.includes('idone') || t.includes('bia')) return 'badge-red';
    return 'badge-gray';
  }
}
